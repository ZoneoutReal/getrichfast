...
const mark = (size) => `
  ...
  .mark s { position: absolute; top: 26%; width: ${Math.max(2, Math.round(size * 0.09))}px; border-radius: 4px; background: #fff; }
  .mark .k1 { left: 26%; height: 34%; } .mark .k2 { left: 44%; height: 48%; } .mark .k3 { left: 62%; height: 34%; }
  .mark u { position: absolute; left: 18%; right: 18%; top: 52%; height: ${Math.max(2, Math.round(size * 0.05))}px; border-radius: 4px; background: rgba(255,255,255,0.6); }
`;
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="mark"><s class="k1"></s><s class="k2"></s><s class="k3"></s><u></u></div>
  <h1>FocusFence</h1>
  <div class="tag">Your time. Fenced off.</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_blocked.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="left">
    <div class="brand"><div class="mark"><s class="k1"></s><s class="k2"></s><s class="k3"></s><u></u></div><b>FocusFence</b></div>
    <h1>Your time. Fenced off.</h1>
    <div class="tag">Block distracting sites with sessions &amp; schedules. 100% local — your browsing stays yours.</div>
    <div class="chips"><span class="chip">⏱️ Focus sessions</span><span class="chip">🔒 No account</span><span class="chip">💰 Pay once</span></div>
  </div>
