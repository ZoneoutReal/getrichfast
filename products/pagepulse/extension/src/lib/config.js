// PagePulse shared configuration.
// EXTPAY_ID: set to your extension's ID from https://extensionpay.com after
// registering it (see FOUNDER_SETUP.md). While null, all payment UI is
// hidden and Pro features stay locked with a "coming soon" note instead of a
// purchase flow.
globalThis.PAGEPULSE_CONFIG = {
  EXTPAY_ID: null,
  PRO_PRICE_LABEL: "$9 one-time",
  HOMEPAGE: "https://zoneoutreal.github.io/getrichfast/pagepulse/",
  FREE_PRESETS: [10, 30, 60, 300, 900, 1800],
  MIN_INTERVAL: 2
};
