...
const mark = (size) => `
  ...
  .mark i { position: absolute; inset: 24%; border: ${Math.max(2, size * 0.055)}px solid #fff;
            border-right-color: transparent; border-bottom-color: transparent; border-radius: ${size * 0.07}px; }
  .mark u { position: absolute; right: 24%; bottom: 24%; width: ${size * 0.16}px; height: ${size * 0.16}px;
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="mark"><i></i><u></u></div>
  <h1>SnipShot</h1>
  <div class="tag">Screenshot. Annotate. Done.</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_editor-annotated.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ...
  img { width: 100%; border-radius: 14px; box-shadow: 0 30px 60px rgba(20, 10, 60, 0.45); }
</style></head><body>
  <div class="left">
    <div class="brand"><div class="mark"><i></i><u></u></div><b>SnipShot</b></div>
    <h1>Mark up any screenshot in seconds.</h1>
    <div class="tag">Arrows, boxes, text, blur — then copy or download. 100% local &amp; private.</div>
    <div class="chips"><span class="chip">⚡ Instant</span><span class="chip">🔒 No upload</span><span class="chip">💰 Pay once</span></div>
  </div>
