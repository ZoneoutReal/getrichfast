...
const mark = (size) => `
  .mark { width: ${size}px; height: ${size}px; border-radius: 22%; position: relative;
          ...
          display: flex; align-items: center; justify-content: center; gap: ${Math.max(2, Math.round(size * 0.06))}px;
          font-weight: 700; font-size: ${Math.round(size * 0.42)}px; color: #fff;
          font-family: ui-monospace, Menlo, monospace; }
  .mark i { width: ${Math.max(3, Math.round(size * 0.1))}px; height: ${Math.max(3, Math.round(size * 0.1))}px; border-radius: 50%; background: #fff; }
`;
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  <h1>JSONPeek</h1>
  <div class="tag">JSON you can actually read.</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_tree.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="left">
    <div class="brand"><div class="mark">{<i></i>}</div><b>JSONPeek</b></div>
    <h1>JSON you can actually read.</h1>
    <div class="tag">Collapsible tree, search, paths, queries, CSV, diff — fast and 100% local.</div>
    <div class="chips"><span class="chip">⚡ One click</span><span class="chip">🔒 No account</span><span class="chip">💰 Pay once</span></div>
  </div>
