...
const mark = (size) => `
  .mark { width: ${size}px; height: ${size}px; border-radius: 22%; position: relative;
          background: rgba(255,255,255,0.16); border: 1.5px solid rgba(255,255,255,0.35); overflow: hidden; }
  .mark s { position: absolute; left: 24%; height: ${Math.max(2, size * 0.055)}px; border-radius: 4px; background: #fff; }
  .mark .s1 { top: 24%; width: 52%; } .mark .s2 { top: 41%; width: 36%; } .mark .s3 { top: 58%; width: 24%; }
  .mark u { position: absolute; right: 14%; bottom: 10%; width: ${size * 0.3}px; height: ${size * 0.36}px;
            background: #fde047; clip-path: polygon(56% 0, 12% 56%, 40% 56%, 30% 100%, 88% 40%, 55% 40%); }
`;
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="mark"><s class="s1"></s><s class="s2"></s><s class="s3"></s><u></u></div>
  <h1>MockFill</h1>
  <div class="tag">Every form. One keystroke. Filled.</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_form-filled.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="left">
    <div class="brand"><div class="mark"><s class="s1"></s><s class="s2"></s><s class="s3"></s><u></u></div><b>MockFill</b></div>
    <h1>Fill test forms in one keystroke.</h1>
    <div class="tag">Realistic fake data for devs &amp; QA — smart detection, custom rules, test cards. 100% local.</div>
    <div class="chips"><span class="chip">⚡ Alt+Shift+F</span><span class="chip">🔒 No account</span><span class="chip">💰 Pay once</span></div>
  </div>
