...
const mark = (size) => `
  .mark { width: ${size}px; height: ${size}px; border-radius: 24%; position: relative;
          ...
          color: #fff; font-size: ${Math.round(size * 0.52)}px; font-weight: 800;
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  body {
    ...
    background:
      ...
      linear-gradient(135deg, #0ea5a3, #2563eb);
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  h1 { font-size: 42px; letter-spacing: -0.02em; font-weight: 800; }
  .tag { font-size: 16.5px; max-width: 340px; }
</style></head><body>
  <div class="mark">R</div>
  <h1>Recall</h1>
  <div class="tag">Search your browsing by content — 100% local</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_popup_hero.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ${baseCSS(1400, 560)} ${mark(58)}
  body { padding: 0 90px; gap: 60px; }
  .left { flex: 1; }
  .brand { display: flex; align-items: center; gap: 14px; margin-bottom: 26px; }
  .brand b { font-size: 34px; letter-spacing: -0.01em; }
  h1 { font-size: 52px; line-height: 1.1; letter-spacing: -0.022em; font-weight: 800; margin-bottom: 18px; }
  ...
  .shotwrap { flex: none; width: 430px; display: flex; align-items: center; }
  img { width: 100%; border-radius: 16px; box-shadow: 0 30px 60px rgba(6, 40, 55, 0.45); }
</style></head><body>
  <div class="left">
    <div class="brand"><div class="mark">R</div><b>Recall</b></div>
    <h1>Search your history by what's on the page.</h1>
    <div class="tag">Full-text search of every article you've read — ranked, with snippets. Nothing leaves your device.</div>
    <div class="chips"><span class="chip">🔍 Content search</span><span class="chip">🔒 100% local</span><span class="chip">💰 Pay once</span></div>
  </div>
