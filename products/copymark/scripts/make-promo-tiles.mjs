...
const mark = (size) => `
  .mark { width: ${size}px; height: ${size}px; border-radius: 22%; position: relative;
          ...
          font-weight: 800; font-size: ${Math.round(size * 0.5)}px; color: #fff; }
  .mark::after { content: "↓"; position: absolute; right: ${Math.round(size * 0.06)}px; bottom: -${Math.round(size * 0.05)}px;
                 font-size: ${Math.round(size * 0.34)}px; color: #a5f3fc; font-weight: 800; }
`;
...
const smallTile = `<!DOCTYPE html><html><head><style>
  ...
  <h1>CopyMark</h1>
  <div class="tag">The web, as clean Markdown.</div>
</body></html>`;

const shotSrc = "file://" + path.join(outDir, "_convert.png");
const marqueeTile = `<!DOCTYPE html><html><head><style>
  ...
  <div class="left">
    <div class="brand"><div class="mark">M</div><b>CopyMark</b></div>
    <h1>Copy anything as clean Markdown.</h1>
    <div class="tag">Headings, lists, links, code, tables — tidy Markdown for Obsidian &amp; Notion. 100% local.</div>
    <div class="chips"><span class="chip">⚡ Alt+Shift+M</span><span class="chip">🔒 No account</span><span class="chip">💰 Pay once</span></div>
  </div>
